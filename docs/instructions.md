...
La página quedó organizada por tipo de archivo:

- HTML público en la raíz: `index.html`, `gallery.html`, `brand.html`.
- Estilos en `css/styles.css`.
- JavaScript general en `js/`.
- Módulos funcionales en `js/components/`.
...
- En archivos HTML, cargar estilos con `css/styles.css`.
- En archivos HTML, cargar scripts desde `js/` o `js/components/` según corresponda.
- En `js/data.js`, las imágenes mantienen rutas desde la raíz: `assets/images/...`.
- En `css/styles.css`, las imágenes se referencian desde la carpeta CSS: `../assets/images/...`.
- Usar carpetas sin espacios y con guiones, por ejemplo `Galeria-ZS` y `Culto-A-Las-Calles`.
...
│   ├── utils.js               (Utilidades)
│   ├── app.js                 (Lógica principal de la home)
...
**Paso 3: Agregar datos en js/data.js**
...
- Encontrar el producto en `js/data.js` por su `id`
...
- Eliminar el objeto completo del array `productsData` en `js/data.js`
...
|-----------|-----------|-------|------|---------------|
| Remeras | optimized/sm/assets/images/Productos/Remeras-Chombas/ | 600px | 400px | 80KB |
...
- Guardar la imagen en:
  - `assets/images/Galeria-ZS/`
  - `assets/images/optimized/lg/assets/images/Galeria-ZS/`
  - `assets/images/optimized/sm/assets/images/Galeria-ZS/`
...
- Abrir el archivo `js/data.js` y buscar el array `communityGalleryData`.
- Agregar un nuevo objeto al final del array con la ruta de la nueva imagen y un texto alternativo:

```javascript
const communityGalleryData = [
    // ... imágenes existentes ...
    {
        src: "assets/images/Galeria-ZS/GALERIA-ZS-001.jpg",
...
- Eliminar el archivo de las tres ubicaciones necesarias:
  ...
  - `assets/images/optimized/lg/assets/images/Galeria-ZS/`
  - `assets/images/optimized/sm/assets/images/Galeria-ZS/`
- Si existe una referencia en `js/data.js`, quitar la ruta del array de imágenes.
- Verificar en `gallery.html` y `js/components/gallery.js` que no queden referencias directas al nombre eliminado.
...
- Si es necesario, actualizar las rutas en `js/data.js` y en `gallery.html`.
...
**Paso 2: Agregar datos de la colaboración en js/data.js**
...
- Encontrar en `js/data.js` la colaboración por `key`
...
### Imágenes no cargan
- Verificar que las rutas en `js/data.js` sean correctas
...
- Revisar que `js/data.js` tenga formato JSON válido
...
- Verificar que `upcomingCollaborationAvailable = true` en `js/app.js`
